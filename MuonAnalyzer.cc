...
class MuonAnalyzer : public edm::EDAnalyzer {
   ...
   private:
      ...
      std::vector<float> muon_dxybs;  // New: dxy with respect to beam spot
...
MuonAnalyzer::MuonAnalyzer(const edm::ParameterSet& iConfig)
{
    ...
    mtree->Branch("muon_dxybs", &muon_dxybs);  // New branch for dxy with respect to beam spot
    mtree->GetBranch("muon_dxybs")->SetTitle("muon dxy with respect to beam spot");
...
void MuonAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   muon_dxy.clear();
   muon_dxyErr.clear();
   muon_dxybs.clear();  // Clear dxybs vector
   muon_dz.clear();
   muon_dzErr.clear();
   muon_jetidx.clear();
   muon_genpartidx.clear();

   // Get vertices and beam spot after clearing the vectors
...
if (mymuons.isValid()) {
      ...
      math::XYZPoint pv(vertices->begin()->position()); // primary vertex position
      const reco::BeamSpot& beamSpot = *beamSpotHandle; // beam spot position
      ...
      for (reco::MuonCollection::const_iterator itmuon = mymuons->begin(); itmuon != mymuons->end(); ++itmuon) {
         if (itmuon->pt() > mu_min_pt) {
            ...
            if (trk.isNonnull()) {
               // Calculate dxy and dz with respect to the primary vertex
               ...
               // Calculate dxy with respect to the beam spot (muon_dxybs)
               math::XYZPoint beamSpotPosition(beamSpot.x0(), beamSpot.y0(), beamSpot.z0());
               muon_dxybs.push_back(trk->dxy(beamSpotPosition));  // Store dxy with respect to beam spot
            } else {
               muon_dxy.push_back(-999);
               muon_dxyErr.push_back(-999);
               muon_dz.push_back(-999);
               muon_dzErr.push_back(-999);
               muon_dxybs.push_back(-999); // If no global track, set dxybs to -999
            }
